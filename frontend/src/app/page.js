        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((feature, i) => (
              <div key={i} className="text-center p-6 rounded-xl hover:bg-canvas transition-colors group">
                <div className="w-14 h-14 bg-primary-100 dark:bg-primary-500/15 rounded-xl flex items-center justify-center mx-auto mb-4 group-hover:bg-primary-500 transition-colors">
                  <feature.icon className="w-7 h-7 text-primary-500 dark:text-primary-300 group-hover:text-white transition-colors" />
                </div>
                <h3 className="font-heading font-semibold text-lg mb-2">{feature.title}</h3>
                <p className="text-muted-fg text-sm">{feature.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 bg-canvas">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {loading ? (
            ...
            <p className="text-center text-subtle py-12">No upcoming events at the moment. Check back soon!</p>
          )}
      ...
      <section className="py-20 bg-surface">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {loading ? (
            ...
            <p className="text-center text-subtle py-12">Projects coming soon!</p>
          )}
        </div>
      </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {news.map((article) => (
                <Link key={article._id} href={`/news/${article._id}`} className="card hover:shadow-lg transition-shadow">
                  <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary-100 dark:bg-primary-500/15 text-primary-800 dark:text-primary-300 mb-3 inline-block">
                    {article.category}
                  </span>
                  <h3 className="font-heading font-semibold text-lg text-strong mb-2">{article.title}</h3>
                  <p className="text-muted-fg text-sm line-clamp-3">{article.excerpt || article.content?.substring(0, 150)}</p>
                </Link>
              ))}
            </div>
                ...
                <a key={s._id} href={s.website || '#'} target="_blank" rel="noopener noreferrer" className="group flex flex-col items-center">
                  {s.logo ? (
                    ...
                    <div className="h-16 w-28 bg-muted rounded-lg flex items-center justify-center">
                      <span className="text-faint font-medium text-sm">{s.name}</span>
                    </div>
                  )}
                  <span className="text-xs text-faint mt-2 group-hover:text-primary-500 dark:group-hover:text-primary-200 transition-colors">{s.name}</span>
                </a>
              ))}
            </div>
          </div>
