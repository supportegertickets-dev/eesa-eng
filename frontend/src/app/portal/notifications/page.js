    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-fg text-sm mt-1">Stay updated with EESA announcements</p>
        </div>
        <div className="flex gap-2">
          {unreadCount > 0 && (
            <button onClick={handleMarkAllRead} className="px-4 py-2 text-sm font-medium text-primary-500 dark:text-primary-300 border border-primary-500 rounded-lg hover:bg-primary-50 dark:hover:bg-primary-500/10 flex items-center gap-1">
          ...
          {notifications.map((n) => {
            ...
            return (
              <div key={n._id} className={`card flex items-start gap-3 ${read ? 'opacity-60' : 'border-l-4 border-l-primary-500'}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${read ? 'bg-muted' : 'bg-primary-100 dark:bg-primary-500/15'}`}>
                  <Icon className={`w-5 h-5 ${read ? 'text-faint' : 'text-primary-500 dark:text-primary-300'}`} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`font-medium ${read ? 'text-muted-fg' : 'text-strong'}`}>{n.title}</p>
                  <p className="text-sm text-muted-fg mt-0.5">{n.message}</p>
                ...
                <div className="flex items-center gap-1">
                  {!read && (
                    <button onClick={() => handleMarkRead(n._id)} className="p-1.5 text-primary-500 dark:text-primary-300 hover:bg-primary-50 dark:hover:bg-primary-500/10 rounded-lg" title="Mark read">
                      <HiCheck className="w-4 h-4" />
                    </button>
                  )}
                  {isAdmin && (
                    <button onClick={() => handleDelete(n._id)} className="p-1.5 text-faint hover:text-red-600 dark:hover:text-red-200 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg" title="Delete">
  ...
  return (
    <form onSubmit={handleSubmit} className="card mb-6 border-2 border-primary-200 dark:border-primary-500/30">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-body mb-1">Target</label>
